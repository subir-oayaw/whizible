...
import {DocCategory_Section} from "./DummyData"; // Import the dummy data
...
import Drawer_DocCategory from "./Drawer_DocCategory";
import Accor_DocCategory from "./Accor_DocCategory";





const DocCategory_Table = ({ index }) => {
  ...
  const [individualChecks, setIndividualChecks] = useState(DocCategory_Section.map(() => false));
  
    useEffect(() => {
      // Assuming DocCategory_Section is defined somewhere and passed as props or imported
      setData(DocCategory_Section);
    }, []);
    ...
    <div className='mx-4 mt-3'>
         <div className="row align-items-end mb-3">
              ...
              <Drawer_DocCategory visible={DrawerVisible} onClose={() => setDrawerVisible(false)}/>
             </div>
        {showForm && <Accor_DocCategory className='mb-3' onClose={handleCloseForm} />}
        <Box sx={{height:340}}>
        ...
        <TableBody>
          {currentData.map((healthsheet, index) => (
            <TableRow
              ...
              <TableCell align="center"><Link href="javascript:;" id="ClearSearchBtn" className="closelink" onClick={() => setDrawerVisible(true)}>{healthsheet.CategoryType}</Link></TableCell>
              <TableCell align="center">{healthsheet.Direc_Name}</TableCell>
...
export default DocCategory_Table
