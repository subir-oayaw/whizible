...
import {CountryInformation_Section} from "./DummyData"; // Import the dummy data
...
import Accor_Country from "./Accor_Country";
import Drawer_Country from "./Drawer_Country";



const Country_Table = ({ index }) => {
  ...
  const [individualChecks, setIndividualChecks] = useState(CountryInformation_Section.map(() => false));
  
    useEffect(() => {
      // Assuming CountryInformation_Section is defined somewhere and passed as props or imported
      setData(CountryInformation_Section);
    }, []);
    ...
    <div className='mx-4 mt-3'>
         <div className="row align-items-end mb-3">
              ...
              <Drawer_Country visible={DrawerVisible} onClose={() => setDrawerVisible(false)}/>
             </div>
        {showForm && <Accor_Country className='mb-3' onClose={handleCloseForm} />}
        <Box sx={{height:340}}>
          ...
          {currentData.map((healthsheet, index) => (
            <TableRow
              ...
              style={{ backgroundColor: index % 2 === 0 ? "#fff" : "#f9f9f9" }}>
                <TableCell></TableCell>
              <TableCell align="left"><Link href="javascript:;" id="ClearSearchBtn" className="closelink" onClick={() => setDrawerVisible(true)}>{healthsheet.Country }</Link></TableCell>
