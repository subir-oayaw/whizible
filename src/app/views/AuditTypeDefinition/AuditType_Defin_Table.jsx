...
import {AuditTypeDefiInfo_Section} from "./DummyData"; // Import the dummy data
...
import Drawer_AuditType_Defin from "./Drawer_AuditType_Defin";
import Accor_AuditType from "./Accor_AuditType";




const AuditType_Defin_Table = ({ index }) => {
  ...
  const [individualChecks, setIndividualChecks] = useState(AuditTypeDefiInfo_Section.map(() => false));
  
    useEffect(() => {
      // Assuming AuditTypeDefiInfo_Section is defined somewhere and passed as props or imported
      setData(AuditTypeDefiInfo_Section);
    }, []);
    ...
    <div className='mx-4 mt-3'>
         <div className="row align-items-end mb-3">
              ...
              <Drawer_AuditType_Defin visible={DrawerVisible} onClose={() => setDrawerVisible(false)}/>
             </div>
        {showForm && <Accor_AuditType className='mb-3' onClose={handleCloseForm} />}
        <Box sx={{height:340}}>
   <TableContainer component={Paper} className="mt-3">
      <Table>
        <TableHead>
        <TableRow>
            <TableCell align="center">Audit Type</TableCell>
        ...
        <TableBody>
          {currentData.map((healthsheet, index) => (
            <TableRow
              ...
              <TableCell align="center"><Link href="javascript:;" id="ClearSearchBtn" className="closelink" onClick={() => setDrawerVisible(true)}>{healthsheet.AuditType}</Link></TableCell>
              <TableCell align="center">{healthsheet.Description}</TableCell>
...
export default AuditType_Defin_Table
