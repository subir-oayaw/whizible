...
import {DepartmentMaster_Section} from "./DummyData"; // Import the dummy data
...
import Drawer_DeptMaster from "./Drawer_DeptMaster";
import Accor_DeptMaster from "./Accor_DeptMaster";



const DeptMaster_Table = ({ index }) => {
  ...
  const [individualChecks, setIndividualChecks] = useState(DepartmentMaster_Section.map(() => false));
  
    useEffect(() => {
      // Assuming DepartmentMaster_Section is defined somewhere and passed as props or imported
      setData(DepartmentMaster_Section);
    }, []);
    ...
    <div className='mx-4 mt-3'>
         <div className="row align-items-end mb-3">
              ...
              <Drawer_DeptMaster visible={DrawerVisible} onClose={() => setDrawerVisible(false)}/>
             </div>
        {showForm && <Accor_DeptMaster className='mb-3' onClose={handleCloseForm} />}
        <Box sx={{height:340}}>
        ...
        <TableBody>
          {currentData.map((healthsheet, index) => (
            <TableRow
              ...
              <TableCell>{healthsheet.ShortName }</TableCell>
              <TableCell align="left"><Link href="javascript:;" id="ClearSearchBtn" className="closelink" onClick={() => setDrawerVisible(true)}> {healthsheet.Department }</Link></TableCell>
...
export default DeptMaster_Table
